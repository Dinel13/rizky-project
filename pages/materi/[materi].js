...
import MateriItem from "components/materi/MateriItem";
...
function Materi() {
  ...
  useEffect(() => {
    const getAllMateri = async (mat) => {
      try {
        setLoading(true);
        const res = await fetch(`/api/materi/` + mat);
        const data = await res.json();
        if (!res.ok) {
          throw new Error(data.message || defaultError);
        }
        setData(data.data);
        data && data.data && data.data.length > 0 && setCurrent(data.data[0]);
      } catch (err) {
        console.log(err);
