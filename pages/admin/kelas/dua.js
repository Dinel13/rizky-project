...
import { youtubeConfig } from "utils/constant/react-player";
...
const Video = () => {
  ...
  return (
    <div className="flex h-screen wrapper items-start relative">
      <div className="flex flex-col flex-1 relative">
        {/* Main content here */}
        <div>
          <div className="relative ">
            <div className="custom-div relative rounded-xl"></div>
            <div className="absolute inset-0 player-rounded">
              {domLoaded && (
                <ReactPlayer
                  url="/video/ruki.mov"
                  controls={true}
          ...
          <div className="mt-8">
            <div className="text-xl font-semibold text-main border-gray-400 border-b-2 py-2">
              <p className="px-1">Deskrpsi materi</p>
            </div>
            <p className="px-1 mt-2 text-base">{data.deskripsi}</p>
          </div>
        </div>

        {isSidebarHidden && (
          <button
            className="btn-main -mr-6 md:-mr-8 absolute text-base bg-third text-white right-0 top-0 py-2 pl-3 pr-4 font-semibold z-10 flex items-center gap-2"
      ...
      <div
        className={`flex flex-col w-80 bg-third rounded-l-3xl md:relative h-full ml-8 -mr-6 ${
...
export default withAdmin(Video);
